...
from .data_utils import xywhn2xyxy, xyxy2xywh, xyn2xy, segment2box, \
...
class COCODataset:
    ...
    def __init__(
        ...
        assert not (self.return_segments and self.return_keypoints), 'Can not return both segments and keypoints.'
        ...
        self.dataset_column_names = ["sample"]

        self.dataloader_column_names = ['image']
        if self.is_training:
            self.dataloader_column_names.append('labels')
            if self.return_segments:
                self.dataloader_column_names.append('segment')
            if self.return_keypoints:
                self.dataloader_column_names.append('keypoints')
        else:
            self.dataloader_column_names.extend(["img_files", "hw_ori", "hw_scale", "pad"])
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if nl:
                        if self.return_keypoints:
                            assert lb.shape[1] == (5 + self.nkpt * self.ndim), f'labels require {(5 + self.nkpt * self.ndim)} columns each'
    ...
    def __getitem__(self, index):
        ...
        image = sample['img']
        sample['img'] = np.ascontiguousarray(image)
        return sample
    ...
    def mosaic(
        ...
        assert bbox_format == 'xywhn', f'The bbox format should be ltrb, but got {bbox_format}'
    ...
    def random_perspective(
        ...
        h, w = img.shape[:2]
        if h != self.img_size:
            border = sample.pop('mosaic_border', None) or border
        ...
        if n:
            ...
            if use_segments:  # warp segments
                ...
                new_bboxes = np.stack([segment2box(xy, width, height) for xy in segments], 0)
    ...
    def mixup(self, sample, alpha=32.0, beta=32.0, pre_transform=None):
        ...
        image, cls, bboxes,  = sample['img'], sample['cls'], sample['bboxes'],
        image2, cls2, bboxes2,  = sample2['img'], sample2['cls'], sample2['bboxes'],

        r = np.random.beta(alpha, beta)  # mixup ratio, alpha=beta=8.0
        image = (image * r + image2 * (1 - r)).astype(np.uint8)
        cls = np.concatenate((cls, cls2), 0)
    ...
    def load_samples(self):
        ...
        # Augment
        sample_labels, sample_images, sample_masks = self._sample_segments(img4, classes4, bboxes4, segments4, probability=0.5)

        return sample_labels, sample_images, sample_masks

    def _sample_segments(self, img, classes, bboxes, segments, probability=0.5):
        # Implement Copy-Paste augmentation https://arxiv.org/abs/2012.07177, labels as nx5 np.array(cls, xyxy)
        n = len(segments)
        sample_labels = []
        sample_images = []
    ...
    def pastein(self, sample, num_sample=30):
        ...
        while len(sample_labels) < num_sample:
            sample_labels_, sample_images_, sample_masks_ = self.load_samples()
    ...
    def label_norm(self, sample, xyxy2xywh_=True):
        ...
        bboxes[:, [1, 3]] /= image.shape[0]  # normalized height 0-1
        bboxes[:, [0, 2]] /= image.shape[1]  # normalized width 0-1
        sample['bbox_format'] = 'xywhn'
        sample['bboxes'] = bboxes
    ...
    def label_pad(self, sample, padding_size=160, padding_value=-1):
        ...
        if nL:
            ...
            labels_out[: min(nL, padding_size), 1:2] = cls[: min(nL, padding_size), 0:1]
            labels_out[: min(nL, padding_size), 2:] = bboxes[: min(nL, padding_size), :]

        sample['labels'] = labels_out
    ...
    def format_segments(self, sample, mask_overlap, mask_ratio):
    ...
    @staticmethod
    def test_collate_fn(batch_samples, batch_info):
        imgs = [sample['img'] for sample in batch_samples]
        path = [sample['im_file'] for sample in batch_samples]
        hw_ori = [sample['ori_shape'] for sample in batch_samples]
        hw_scale = [sample['hw_scale'] for sample in batch_samples]
        pad = [sample['hw_pad'] for sample in batch_samples]
        return (
            np.stack(imgs, 0),
            path,
            np.stack(hw_ori, 0),
