...
def train(args):
    ...
    # load data
    import numpy as np
    img = np.load("./npy_files/batch_img.npy")
    gt_mask = np.load("./npy_files/batch_mask.npy")
    _idx = np.load("./npy_files/batch_idx.npy")
    _cls = np.load("./npy_files/batch_cls.npy")
    _box = np.load("./npy_files/batch_bboxes.npy")
    gt_boxes = np.ones((16, 160, 6), dtype=np.float32) * -1
    for i in range(16):
        _select = (_idx == i)
